...
FROM Facilities 
    WHERE membercost != 0 
    LIMIT 0 , 30
-- Facilities which charge a fee: [0:Tennis Court 1,1:Tennis Court 2 ,4:Massage Room 1 ,5:Massage Room 2 ,6:Squash Court]
...
FROM Facilities 
    WHERE membercost = 0 
    LIMIT 0 , 30
-- There are 4 facilities which do not charge a fee to members. These include: [2:Badminton Court, 3:Table Tennis, 7:Snooker Table, 8: Pool Table]
...
FROM Members 
    WHERE memid=(SELECT MIN(memid) FROM Members WHERE memid != 0) 
    OR memid=(SELECT MAX(memid) FROM Members);
...
FROM country_club.Bookings b
    ...
    WHERE left( starttime, 10 ) = '2012-09-14' AND m.memid =0
    UNION
...
FROM country_club.Bookings b
    ...
    JOIN country_club.Members m ON m.memid = b.memid
    WHERE left( starttime, 10 ) = '2012-09-14' AND m.memid !=0
    GROUP BY m.memid HAVING cost >30 ORDER BY cost DESC
    LIMIT 0 , 30
